...
/**
 * Cloud Shader — native-WebGL animated clouds (domain-warped billow noise,
 * self-shadowing, atmospheric perspective, drift + bob).
 *
 ...
 *  - DPR capped at 1.5 — decorative, unlike the walkthrough canvas at 3
 *  - reduced-motion now draws a single frame instead of looping on a static image
 *  - added visibilitychange pause and webglcontextlost handling
 * Deva palette + tuning live in components/bg-shader.tsx, not here.
 */
...
export const CloudShader = ({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      // Reduced motion: elapsed is pinned to 0, so every subsequent frame is
      // identical. Draw once and stop rather than burning a rAF loop on it.
    ...
    // This layer is mounted site-wide for the life of the session, so a loop
    // that keeps running in a hidden tab is a real battery cost.
    const onVisibility = () => {
      if (document.hidden) {
        running = false;
    ...
    // Without this a GPU reset leaves a dead canvas for the rest of the
    // session; preventDefault keeps the context restorable.
